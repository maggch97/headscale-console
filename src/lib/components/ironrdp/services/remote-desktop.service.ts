...
// Source: https://github.com/Devolutions/IronRDP/blob/bdde2c76ded7315f7bc91d81a0909a1cb827d870/web-client/iron-remote-desktop/src/services/remote-desktop.service.ts
...
export class RemoteDesktopService {
  ...
  private mousePosition: BehaviorSubject<MousePosition> =
    new BehaviorSubject<MousePosition>({
      x: 0,
      y: 0,
    });
  private changeVisibility: Subject<boolean> = new Subject();
  private sessionEvent: Subject<SessionEvent> = new Subject();
  private scale: BehaviorSubject<ScreenScale> = new BehaviorSubject(
    ScreenScale.Fit as ScreenScale
  );
  private canvas?: HTMLCanvasElement;
  private keyboardUnicodeMode: boolean = false;
  private backendSupportsUnicodeKeyboardShortcuts: boolean | undefined =
    undefined;
  ...
  mousePositionObservable: Observable<MousePosition> =
    this.mousePosition.asObservable();
  changeVisibilityObservable: Observable<boolean> =
    this.changeVisibility.asObservable();
  ...
  constructClipboardContentFromText(
    mime_type: string,
    text: string
  ): ClipboardContent {
  ...
  constructClipboardContentFromBinary(
    mime_type: string,
    binary: Uint8Array
  ): ClipboardContent {
  ...
  updateMousePosition(position: MousePosition) {
    this.doTransactionFromDeviceEvents([
      this.module.DeviceEvent.mouse_move(position.x, position.y),
    ]);
  ...
  connect(config: Config): Observable<NewSessionInfo> {
    ...
    sessionBuilder.data_channel(config.dataChannel);
    ...
    if (this.onRemoteClipboardChanged != null && this.enableClipboard) {
      sessionBuilder.remote_clipboard_changed_callback(
        this.onRemoteClipboardChanged
      );
    }
    if (this.onRemoteReceivedFormatList != null && this.enableClipboard) {
      sessionBuilder.remote_received_format_list_callback(
        this.onRemoteReceivedFormatList
      );
    }
    if (this.onForceClipboardUpdate != null && this.enableClipboard) {
      sessionBuilder.force_clipboard_update_callback(
        this.onForceClipboardUpdate
      );
    }

    if (config.desktopSize != null) {
      sessionBuilder.desktop_size(
        this.module.DesktopSize.init(
          config.desktopSize.width,
          config.desktopSize.height
        )
      );
    }
          ...
          .pipe(
            ...
            map((termination_info: SessionTerminationInfo) => {
              ...
              this.raiseSessionEvent({
                type: SessionEventType.TERMINATED,
                data:
                  "Session was terminated: " + termination_info.reason() + ".",
              });
            })
          )
          .subscribe();
  ...
  onClipboardChangedEmpty(): Promise<void> {
    const onClipboardChangedPromise = async () => {
      await this.session?.on_clipboard_paste(
        this.module.ClipboardTransaction.init()
      );
    };
    return onClipboardChangedPromise();
  }
  ...
  private supportsUnicodeKeyboardShortcuts(): boolean {
    ...
    if (this.session?.supports_unicode_keyboard_shortcuts) {
      this.backendSupportsUnicodeKeyboardShortcuts =
        this.session?.supports_unicode_keyboard_shortcuts();
      return this.backendSupportsUnicodeKeyboardShortcuts;
    }
  ...
  private sendKeyboard(evt: KeyboardEvent) {
    ...
    if (!evt.repeat || (!isModifierKey && !isLockKey)) {
      ...
      if (this.keyboardUnicodeMode && unicodeEvent && keyEvent) {
        ...
        const isUnicodeCharacter =
          Number.isNaN(keyCode) && evt.key.length === 1;
  ...
  private updateModifierKeyState(evt: KeyboardEvent) {
    ...
    if (this.modifierKeyPressed.indexOf(modKey) === -1) {
      ...
      this.modifierKeyPressed.splice(
        this.modifierKeyPressed.indexOf(modKey),
        1
      );
    }
  }
